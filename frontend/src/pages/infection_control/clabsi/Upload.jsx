...
function ClabsiUpload({ language }) {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    for (let dept of departments) {
      if (!denominators[dept] || Number(denominators[dept]) <= 0) {
        setError(
          ar
            ? `يرجى إدخال أيام القسطرة لقسم ${dept}`
            : `Please enter catheter days for ${dept}`
        );
        return;
      }
    }
    ...
    // Convert denominators to numbers
    const numericDenominators = {};
    departments.forEach((dept) => {
      numericDenominators[dept] = Number(denominators[dept]);
    });

    formData.append('denominators', JSON.stringify(numericDenominators));
  ...
  }, [quarter, year, navigate, ar,departments]);
  ...
  return (
    <div className={styles.uploadContainer}>
      ...
      >
        <div className={styles.headerContent}>
          ...
          <div className={styles.headerText}>
            <h2>{ar ? 'رفع بيانات CLABSI' : 'Upload CLABSI Data'}</h2>
            <p>
              {ar
                ? 'ارفع ملف Excel يحتوي على بيانات حالات CLABSI'
                : 'Upload an Excel file containing CLABSI case data'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className={styles.uploadCard}>
        ...
        {/* Catheter Days per Department */}
        <div className={styles.inputSection}>
          <label className={styles.inputLabel}>
            🏥 {ar ? 'أيام القسطرة لكل قسم' : 'Catheter Days per Department'}
          </label>
